...
export function AgentDashboard() {
  ...
  const [currentPatient, setCurrentPatient] = useState<Ticket | null>(counter?.currentTicket || null)
  ...
  const handleCallNext = () => {
    ...
    if (nextTicket) {
      ...
      setLastAction(`Ticket ${nextTicket.number} appele`)
  ...
  const handleMarkAbsent = () => {
    if (currentPatient) {
      ...
      setLastAction(`Ticket ${currentPatient.number} marque absent`)
  ...
  const handleComplete = () => {
    if (currentPatient) {
      ...
      setLastAction(`Ticket ${currentPatient.number} termine`)
      setCurrentPatient(null)
    }
  }
  ...
  const handleToggleCounter = (open: boolean) => {
    toggleCounter(open)
    setLastAction(open ? "Guichet ouvert" : "Guichet ferme")
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background p-6">
        <Card className="max-w-md w-full text-center p-8">
          ...
          <h2 className="text-xl font-bold text-foreground mb-2">Aucun guichet assigne</h2>
          <p className="text-muted-foreground">
            Vous n&apos;etes pas assigne a un guichet. Contactez votre administrateur.
          </p>
        </Card>
      </div>
    )
        ...
        <div className="mb-6 grid gap-4 grid-cols-3">
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-foreground">{agent.ticketsServedToday}</p>
          ...
          <div className="lg:col-span-3">
            ...
            >
              <Card className="overflow-hidden border-0 shadow-xl">
                <div className={`p-6 ${currentPatient ? "bg-primary" : "bg-muted"} text-${currentPatient ? "primary-foreground" : "foreground"}`}>
                  <div className="flex items-start justify-between">
                      ...
                      <AnimatePresence mode="wait">
                        {currentPatient ? (
                          ...
                          >
                            ...
                            <p className="mt-2 text-lg">{currentPatient.userName}</p>
                          </motion.div>
                        ) : (
                          <motion.div
                            initial={{ opacity: 0 }}
                          ...
                          >
                            ...
                            <p className="mt-2 text-sm text-muted-foreground">
                              Cliquez sur &quot;Suivant&quot; pour appeler
                            </p>
                          </motion.div>
                        )}
                      </AnimatePresence>
                    ...
                    {currentPatient && (
                      <Badge className="bg-white/20 text-white border-0">
                        {currentPatient.status === "called" ? "Appele" : "En cours"}
                      </Badge>
                    )}
                  </div>
                </div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="lg"
                      className="h-20 flex-col gap-2 border-2 border-amber-500 text-amber-600 hover:bg-amber-500 hover:text-white"
                    >
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="lg"
                      className="h-20 flex-col gap-2 border-2 border-red-500 text-red-500 hover:bg-red-500 hover:text-white"
                    >
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="lg"
                      className="h-20 flex-col gap-2 border-2 border-primary text-primary hover:bg-primary hover:text-primary-foreground"
                    >
                        ...
                        >
                          ...
                          <div className="text-right">
                            <p className="text-xs text-muted-foreground">
                              {Math.round((Date.now() - new Date(ticket.createdAt).getTime()) / 60000)} min
                            </p>
                            {index === 0 && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-foreground">
                    {counter.isActive ? "Guichet actif" : "Guichet ferme"}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {counter.isActive 
                      ? "Vous recevez des patients" 
                      : "Les patients ne sont pas diriges vers ce guichet"
                    }
                  </p>
                </div>
              </div>
