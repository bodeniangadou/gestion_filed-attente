...
import {
  ...
  FileText, TrendingUp, TrendingDown, Minus, ChevronDown, Check
} from "lucide-react"
...
// ─── PDF export (jsPDF + autotable — direct download, no popup) ─────────────

async function exportToPDF(sections: ExportSection[], data: ExportData, label: string) {
  // Dynamic imports so Next.js doesn't bundle on SSR
  ...
  if (sections.includes("serviceStatus")) {
    ...
    autoTable(doc, {
      ...
      body: data.serviceStatus.map(r => [r.name, r.active ? "Actif" : "Fermé", r.activeCounters, r.waiting, r.total]),
      theme: "striped",
      headStyles: { fillColor: [29, 158, 117] },
      margin: { left: 14, right: 14 },
    })
  ...
  if (sections.includes("ticketList")) {
    ...
    autoTable(doc, {
      ...
      body: data.ticketList.map(t => [t.number, t.service, t.counter, t.time, t.status]),
      theme: "striped",
      headStyles: { fillColor: [29, 158, 117] },
      margin: { left: 14, right: 14 },
    })
...
// ─── Excel export (SheetJS — real .xlsx, direct download) ───────────────────
...
async function exportToExcel(sections: ExportSection[], data: ExportData, label: string) {
  ...
  if (sections.includes("ticketList")) {
    const ws = XLSX.utils.aoa_to_sheet([
      ...
      ...data.ticketList.map(t => [t.number, t.service, t.counter, t.time, t.status]),
    ])
...
export function AdminDashboard() {
  ...
  // Computed stats
  // Avant : Utilisation de comparaisons strictes qui bloquaient
// Après : Modifié pour scanner les statuts réels sans erreur de type
// Des filtres ultra-tolérants pour intercepter tous les formats de statuts
...
const prevWaiting     = prevTickets.filter(t => (t.status as string) === "waiting").length
  ...
  const ticketsPerService = useMemo(() => {
    ...
    periodTickets.forEach(t => {
      if (!map[t.service.id]) map[t.service.id] = { name: t.service.name, count: 0, waiting: 0, done: 0 }
      map[t.service.id].count++
      if ((t.status as string) === "waiting") map[t.service.id].waiting++
      if ((t.status as string) === "completed" || (t.status as string) === "serving") map[t.service.id].done++
    })
  ...
  const exportData: ExportData = useMemo(() => ({
    ...
    serviceStatus: services.map(s => ({
      ...
      waiting: periodTickets.filter(t => t.service.id === s.id && t.status === "waiting").length,
      total: periodTickets.filter(t => t.service.id === s.id).length,
    })),
...
counter: (t as any).counter?.name || "—",
    })),
  }), [periodTickets, waitingCount, completedCount, cancelledCount, stats, activeAgents, agents, services, counters, ticketsPerService])
              ...
              <CardContent>
                <div className="space-y-2">
                  {services.slice(0, 5).map(service => {
                    const activeCountersCount = counters.filter(c => c.serviceId === service.id && c.isActive).length
                    const queueCount = periodTickets.filter(t => t.service.id === service.id && t.status === "waiting").length
                  ...
                  {recentActivity.map((activity, index) => {
                    ...
                    return (
                      <div key={index} className="flex items-start gap-3">
                        <div className={`mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full ${activity.type === "ticket" ? "bg-blue-100 text-blue-600" :
